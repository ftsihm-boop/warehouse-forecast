...
def adapt_to_data(
    ...
    """
    Дообучает модель на данных пользователя и возвращает ту версию,
    которая на этих данных работает лучше.

    ДООБУЧЕНИЕ ИДЁТ ВСЕГДА, А НЕ ТОЛЬКО ПРИ ПРОВАЛЕ ПРОВЕРКИ.
    Раньше было иначе: если модель проходила проверку по точности, она
    оставалась как есть — то есть файл пользователя она только смотрела,
    но ничему на нём не училась. Это оказалось ошибкой. Проверка отвечает
    на вопрос «насколько точен прогноз», но не на вопрос «выгодно ли по
    нему закупать». Модель с ошибкой 9.5 % спокойно проходила проверку и
    при этом теряла деньги: её доверительный интервал был откалиброван
    под чужой ассортимент, страхового запаса не хватало, и экономический
    расчёт уходил в минус.

    Поэтому модель дообучается на каждом загруженном файле. Обучение
    идёт только на части истории до проверочного окна, а сравнение
    «стало лучше или хуже» — на окне, которого не видела ни исходная
    модель, ни дообученная. Если лучше не стало, возвращается исходная:
    предсказуемое поведение важнее попытки улучшить любой ценой.
    """
    report = check_fit(df, model, horizon)
    if model is None:
        return None, report
    if not report.checked and not force:
        # проверить не на чем — значит и убедиться в пользе дообучения
        # нечем; молча менять модель в такой ситуации нельзя
        return model, report

    was_suitable = report.suitable
    ...
    # Дообучение не улучшило модель. Смысл этого разный в двух случаях,
    # и путать их нельзя: одно дело «модель и так настроена под ваши
    # данные», совсем другое — «модель им не подходит и починить её
    # этим файлом не вышло».
    if was_suitable:
        report.action = "verified"
        report.message += (" Дообучение на этом файле улучшения не дало — "
                           "модель уже настроена под такой ассортимент.")
    else:
